<script lang="ts">
  ...
  // otherwise the breadcrumb flashes for a frame at the 60px boundary.
...
<header>
      ...
      <div class="hidden md:flex items-center gap-4">
        <ul role="navigation" aria-label="Main" class="flex flex-row gap-6 items-center">
          {#each menuItems as menuItem}
            <li>
              <Button type="nav" active={menuItem.current} label={menuItem.label} url={menuItem.uri} />
            </li>
          {/each}
        </ul>
  ...
  <!-- Collapsed (mobile): a matching circle around the hamburger so the menu
       stays reachable. Fades in once the bar has finished collapsing. -->
  {#if collapsed}
    <div
    ...
    >
      {#each menuItems as menuItem, i}
        <li class="menu-item" style="animation-delay: {100 + i * 50}ms;">
          <Button type="nav" active={menuItem.current} label={menuItem.label} url={menuItem.uri} onclick={() => (open = false)} />
        </li>
      {/each}
      <li class="menu-item mt-20" style="animation-delay: {200 + menuItems.length * 50}ms;">
