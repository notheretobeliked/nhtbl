...
import type { ImageObject, ImageSize } from '$lib/types/wp-types.ts'
  ...
  const src = findImageSizeData('sourceUrl', imageObject.mediaDetails.sizes, imageSize) // Note: Ensure the property name matches exactly, JavaScript is case-sensitive.
...
<img loading={lazy ? 'lazy' : undefined} class="w-full h-full object-{fit} {extraClasses}" src={src} alt={imageObject.altText} width={width} height={height} srcset={getSrcSet(imageObject.mediaDetails.sizes)} sizes="(max-width: 600px) 480px, 800px" />
